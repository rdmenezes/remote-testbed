...
implementation
{
	...
	{
		return;//call Uart.putSync(0x10 | (0x0F & ledStatus));
	}
	...
	*/
	 ...
	 */
	command bool ConsoleOut.dumpHex(const uint8_t ptr[], const uint8_t countar, const char * sep)
	{
		int i;
		for (i = 0; i < countar; i++) {
			if (i != 0) {
	...
	async event  bool Uart.get(uint8_t consoleData)
	{
