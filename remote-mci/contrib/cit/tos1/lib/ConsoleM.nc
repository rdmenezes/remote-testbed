...
implementation
{
	...
	*/
	command int ConsoleOut.print(const char *str)
	{
		...
		while (*str != 0) {
			if (call Uart.putBuffered(*str)) {
				//call Uart.put(*str);
				str++;
				countret++;
			} else {
				// Wait until buffer space is available (triggered by an interrupt).
				TOSH_sleep();
			}
		}
	...
	command int ConsoleOut.printStr(const char *str, uint16_t length)
	{
		...
		uint8_t i = 0;
		while (i < length) {
			if (call Uart.putBuffered(str[i])) {
				i++;
				countret++;
			} else {
				// Wait until buffer space is available (triggered by an interrupt).
	 ...
	 */
	command result_t ConsoleOut.dumpHex(const uint8_t ptr[], const uint8_t countar, const char * sep)
	{
		int i;
		for (i = 0; i < countar; i++) {
			if (i != 0) {
	...
	{		
		// Never called, since we are using buffered output.
		return SUCCESS;
	}

	async default event result_t ConsoleIn.get(uint8_t consoleData)
	{
		(void) consoleData;
		return SUCCESS;
	}
	...
	async event result_t Uart.get(uint8_t consoleData)
	{
		signal ConsoleIn.get(consoleData);
		return SUCCESS;
	}
}
